...
{
  DBG_PRINT(GRN "kdb_read_tcp_messages" RST ": have sock_fd {}, table-filter: {}", conn.sock_fd(), sub.tables() | std::views::join_with(',') | std::ranges::to<std::string>());
  ...
  do {
    ...
    wr_off += io_res.value();
    TRA_PRINT(GRN "kdb_read_tcp_messages" RST ": read {} bytes on FD {}, wr_off is {}", io_res.value(), conn.sock_fd(), wr_off);
    while (wr_off - rd_off > 0) {
      ...
      if (len > 0) {
        ...
        io_res = ::mg7x::io::write(sub.dst_jnl_fd(), ary.data() + SZ_MSG_HDR + rd_off, len - SZ_MSG_HDR);
        if (!io_res.has_value()) {
