...
TASK_TYPE<std::expected<int,ErrnoMsg>> kdb_subscribe_and_replay(EpollCtl & epoll, const io::TcpConn & conn, const Subscription & sub, TpMsgCounts & counts)
{
  ...
  std::expected<int,int> result = ::mg7x::io::open(src_path.data(), O_RDONLY);
  if (!result.has_value()) {
  ...
  if (msg_count->m_val > 0) {
    int err = filter_jnl(src_fd, static_cast<uint64_t>(msg_count->m_val), sub.dst_jnl_fd(), names, counts);
    if (-1 == err) {
      ERR_PRINT(CYN "kdb_subscribe_and_replay" RST ": while filtering journal messages; closing socket FD {}", conn.sock_fd());
      // not closing journal FD as it may be in use by other subscriptions
      monitor_close(conn.sock_fd());
