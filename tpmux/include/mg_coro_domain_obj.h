...
class Subscription
{
  ...
  KdbJournal m_dst_jnl;

public:
  Subscription(const char *service, const char *username, std::vector<std::string_view> tables, KdbJournal & jnl)
   : m_service(service)
   , m_username(username)
   , m_tables(tables)
   , m_dst_jnl(jnl)
  ...
  std::expected<int,std::string> init_jnl() noexcept { return m_dst_jnl.init(); }
  std::expected<int,std::string> close_jnl() noexcept { return m_dst_jnl.close(); }
  int dst_jnl_fd() const noexcept { return m_dst_jnl.jnl_fd(); }
};
