...
/ ---------------------------------------------------------------------------------8<--
/ Run both client and server with Krb5 authentication with:
...
/ and the server
/ qq src/boot.q krb5 -krb.server 1

/ and client
/ qq src/boot.q krb5

/ nyi: need to add the expiry TTL to the .krb.conns`xpy column on IPC connection
.krb.init:{
  dso:$[1<>count dso:.utl.pathFind[`LD_LIBRARY_PATH;"libmgkrb5.so"]
       ;'"failed to find unique libmgkrb5.so"
       ;hsym`$-3_ first dso
       ]
 ...
 ;.krb.conns:1!flip`fd`src`tgt`xpy`ctx`dir!enlist each (0Ni;"";"";0Np;::;" ")
  ...
  // a hardening guide ;)
 ;rgs:.boot.getargs flip`name`reqd`default!enlist each (`krb.server;0b;0b)
 ;if[rgs`krb.server
    ;.z.pw:.krb.zpw
    ]
 }
...
.krb.onMutualAuth:{[T;K]
  cfd:.utl.zw[]
 ;.log.debug("Have mutual-auth reply from ";T;" on FD ";cfd)
