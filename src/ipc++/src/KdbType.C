...
{
  // Per Claude 2026-06-10; TODO:
  // Reliance on vector::capacity() as the element count — readIntVector
  // (606-628), KdbList::read (945-955), KdbProjection::read (1977-1988),
  // KdbSymbolVector::read (1356-1367). The parse loops run to capacity(), assuming
  // reserve(n)/vector(n) yields capacity()==n. The standard only guarantees >= n;
  // any implementation that over-allocates would make the reader consume extra
  // elements and corrupt the stream. Store the intended count explicitly rather
  // than trusting capacity().
 ...
 , m_attr(attr)
{
  ...
  for (uint64_t i = 0 ; i < vals.size() ; i++) {
    m_data.insert(m_data.end(), vals[i].data(), vals[i].data() + m_locs[i].c_len - SZ_BYTE);
...
WriteResult KdbTable::write(WriteBuf & buf) const
{
  ...
  if (WriteResult wr = m_cols->write(buf); WriteResult::WR_OK != wr)
    return wr;
  // Per Claude 2026-06-10; TODO:
  // KdbDict::wireSz() dereferences possibly-null members:
  // Unlike count()/getKeyType(), it unconditionally calls
  // m_keys->wireSz()/m_vals->wireSz()
  // a default-constructed KdbDict passed to ipcMessageLen crashes.
...
bool KdbIpcMessageReader::readMsgHdr(ReadBuf & buf, ReadMsgResult & result)
{
  if (0 == m_ipc_len) {
    ...
    if (m_compressed) {
      ...
      int8_t *dst;
      try {
        dst = new int8_t[m_msg_len - SZ_MSG_HDR];
      }
...
size_t KdbUtil::writeLoginMsg(std::string_view usr, std::string_view pwd, std::unique_ptr<char[]> & ptr)
{
  ...
  try {
    ...
    std::cerr << "alloc.fail: " << ex.what() << std::endl;
    return 0;
  }
  size_t off = 0;
  ...
  switch(typ) {
    ...
    case KdbType::STEP_DICT:
      if (0 > (len = KdbUtil::ipcPayloadLen(src + 1, rem - 1)))
        return len;
      if (0 > (tmp = KdbUtil::ipcPayloadLen(src + 1 + len, rem - 1 - len)))
        return tmp;
...
constexpr static int64_t msg_len_cmp(const int32_t len, const size_t width, const uint64_t rem)
{
  ...
  if (SZ_VEC_HDR + (uint64_t)ulen * width > rem)
    return -1;
                                 ...
                                 const std::unordered_set<std::string_view> & tbl_names)
{
  ...
  const size_t fln = strnlen(reinterpret_cast<const char*>(src+off), len-off);
  ...
  // TOOD: should probably check strnlen here too:
...
std::expected<KdbJournal,std::string> KdbJournal::init(std::filesystem::path path, const Options & opts)
{
  ...
  if (sbuf.st_size > SZ_MSG_HDR && opts.validate_and_count_upon_init) {
    ...
    auto counter = [](uint64_t ith, const int8_t *_src, uint64_t len) -> int {
      (void)ith; (void)_src; (void)len;
      return 1;
    };
