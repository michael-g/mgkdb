...
static std::unique_ptr<T> byteByByte(const char *hex)
{
	...
	int64_t xlen;
	std::unique_ptr<int8_t[]> ary = fromHex(hex, &xlen);
	if (xlen < 2)
	...
	ReadBuf buf{src, 0};
	for (int64_t i = 1 ; i <= xlen ; i++) {
		buf.rewind();
		buf.setLength(i);
		if (nullptr == ptr) {
			ReadResult rr = KdbUtil::newInstanceFromIpc(buf, &ptr);
			...
			EXPECT_EQ(i == xlen ? ReadResult::RD_OK : ReadResult::RD_INCOMPLETE, rr);
		}
	}
	// TODO evaluate content of buf
	EXPECT_EQ(xlen, ptr->wireSz());
...
static bool testWrite(const KdbBase *obj, const char *exp) {
	...
	else if ((int64_t)wsz != buf.cursorOff()) {
...
TEST(KdbTypeTest, TestKdbSymbolVectorCtorVectorStringView)
{
  // I had some duuuh problems implementing this, so it's got its own test
  std::vector<std::string_view> vec{"Tom", "Dick", "Harry"};
  KdbSymbolVector sym_vec{vec};
  EXPECT_EQ(3, sym_vec.count());
  EXPECT_EQ(std::string_view{"Tom"}, sym_vec.getString(0));
  EXPECT_EQ(std::string_view{"Dick"}, sym_vec.getString(1));
  EXPECT_EQ(std::string_view{"Harry"}, sym_vec.getString(2));
}
...
TEST(KdbTypeTest, TestKdbDict)
{
	...
	for (uint64_t i = 0 ; i < dct->count() ; i++) {
