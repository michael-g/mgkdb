...
// I've left these inline macro declarations in-place because this is a 
// C source file and doesn't fit with the C++ std::format handlers in 
// MgDebug.H. Maybe I'll get to adapting the interface, but at that 
// point it's probably time to use a proper logging library.
...
inline static uint64_t mg_gettime(void)
{
  ...
  if (-1 == clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &t)) {
    ERR_PRINT("clock_gettime: %s", strerror(errno));
    exit(EXIT_FAILURE);
  }
